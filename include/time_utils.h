 ...
 *
 * The pure math helpers below are shared with the unit tests (QEMU env),
 * which cannot exercise the RTC memory / NTP around them.
 */
namespace rtc_drift {

// Drift-learning constants.
constexpr double kMinFactor = 0.90;              // clamp of the learned factor
constexpr double kMaxFactor = 1.10;
constexpr double kLearnAlpha = 0.25;             // EMA smoothing of new samples
constexpr double kMaxRateError = 0.10;           // reject outlier samples (> 10%)
constexpr uint64_t kMinLearnIntervalUs = 45ULL * 60ULL * 1000000ULL;
constexpr double kMaxShiftRatio = 0.10;          // clamp of the post-wake shift
...
inline double updateFactor(double k, double rateError) {
  return clampFactor(k + kLearnAlpha * (sampleFactor(rateError) - k));
}
...
// The shift is clamped to kMaxShiftRatio of the claimed duration as a safety
// bound against a corrupt factor.
inline int64_t wakeShiftUs(uint64_t claimedUs, double k) {
  if (k <= 0.0 || k == 1.0 || claimedUs == 0) {
    return 0;
  }
  double shift = (double)claimedUs * (k - 1.0);
  const double bound = (double)claimedUs * kMaxShiftRatio;
  if (shift > bound) {
    shift = bound;
  } else if (shift < -bound) {
    shift = -bound;
