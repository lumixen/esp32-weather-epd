 ...
 * temporary can never be silently borrowed. read() and peek() return the
 * byte as unsigned (0..255, -1 at EOF) so UTF-8 payloads (the "µ" units
 * markers, Cyrillic alert text) never promote signed chars to unexpected
 * negative values. bytesRead() counts consumed bytes for tests that assert
