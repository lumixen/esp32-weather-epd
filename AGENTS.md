...
- One Unity env: `lolin_d32_qemu` (board `lolin_d32`), run one selected registry configuration at a time. The config is baked into the firmware at compile time, so `test/run_tests.sh` reuses the env once per selected config and selects exactly one compatible root suite with `-f`; the mapping comes from `test/configs/index.yml`. Each root has a single driver and feature-level `.inc` modules, so all cases for that config share one build, boot, and Unity session. The provider combinations intentionally differ; do not collect incompatible roots together. All selected runs execute; the script exits non-zero if any selected run failed. The run must end with all test cases succeeding.
- Test configs are **committed and pinned** (`test/configs/*.yml`); test builds must never depend on the untracked local `config.yml`. Adding a config variant = a new pinned `.yml`, a matching root under `test/src/`, and one entry in `test/configs/index.yml`; do not add another per-config shell or CI block.
- The env compiles `src/` together with the tests (`test_build_src = yes`); `main.cpp` is excluded via `#if !defined(PIO_UNIT_TESTING)`. Test suite discovery and mapping come from `test/configs/index.yml`; each compatible configuration root under `test/src/` contains its `test_<config>.cpp` driver plus feature-level `.inc` modules and fixtures. Add a new configuration variant as a new root and registry entry; do not collect incompatible roots together. `test_dir = test/src` keeps the test infrastructure outside the build. The shared read-only stream helper over a String lives in `test/src/string_stream.h` (include it as `"../string_stream.h"`).
...
- Per-device configs live in `devices/<name>.yml` (gitignored, may contain credentials; only `*.example.yml` templates are tracked). Each is a fully independent config (panel, pins, WiFi, location, MQTT) validated against the same schema. `config.yml` remains the default for plain `pio run`; the QEMU test env defaults to the committed configuration marked `default` in `test/configs/index.yml` instead.
