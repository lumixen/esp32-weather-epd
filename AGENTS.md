...
- One Unity env: `lolin_d32_qemu` (board `lolin_d32`), run for every test config in `test/configs/`. The config is baked into the firmware at compile time, so `test/run_tests.sh` (executed in the container by the docker wrapper) reuses the env once per config and selects exactly one matching root suite with `-f`: `test_openmeteo` for `test/configs/openmeteo.yml` (the env default pinned by `scripts/select_test_env_config.py`), `test_owm` for `ESP32_EPD_CONFIG=test/configs/owm.yml`, `test_noaa` for `ESP32_EPD_CONFIG=test/configs/noaa.yml`, and `test_meteoswiss` for `ESP32_EPD_CONFIG=test/configs/meteoswiss.yml`. Each root has a single driver and feature-level `.inc` modules, so all cases for that config share one build, boot, and Unity session. The provider combinations intentionally differ; bare collection of both roots is not a supported full run. All runs always execute; the script exits non-zero if any failed. The run must end with all test cases succeeding.
- Test configs are **committed and pinned** (`test/configs/*.yml`); test builds must never depend on the untracked local `config.yml`. Adding a config variant = a new `.yml` here + one `pio test` invocation in `test/run_tests.sh`.
- The env compiles `src/` together with the tests (`test_build_src = yes`); `main.cpp` is excluded via `#if !defined(PIO_UNIT_TESTING)`. Test suites are the four configuration roots under `test/src/`: each contains its `test_<config>.cpp` driver plus feature-level `.inc` modules and fixtures. Add a new configuration variant as a new root and a matching `-f <root>` invocation in `test/run_tests.sh`; do not collect incompatible roots together. `test_dir = test/src` keeps the test infrastructure outside the build. The shared read-only stream helper over a String lives in `test/src/string_stream.h` (include it as `"../string_stream.h"`).
...
- The emulator hookup is the docs' multiline `test_testing_command` (each line = executable + args, `this.__env__` = env name): `python3 test/qemu_broker.py --qemu /opt/qemu/... --esptool ${sysenv.HOME}/.platformio/penv/bin/esptool --build-dir ${platformio.build_dir}/${this.__env__}`. The broker merges bootloader + partitions + app into a flash image (esptool `merge_bin`) and boots it with `qemu-system-xtensa`; serial output streams to stdout where PlatformIO's Unity runner parses it.
...
- Per-device configs live in `devices/<name>.yml` (gitignored, may contain credentials; only `*.example.yml` templates are tracked). Each is a fully independent config (panel, pins, WiFi, location, MQTT) validated against the same schema. `config.yml` remains the default for plain `pio run`; the QEMU test env defaults to the committed `test/configs/openmeteo.yml` instead.
...
- Determine the actual author(s) and copyright holder(s) for new work from the file provenance and project policy; do not assume attribution based on the current user or agent.
- A file created entirely from one contributor's original work may list that contributor, subject to project policy. A new filename, file move, reformat, wrapper, translation, or other derived work is not automatically original.
- Inherited or derived files must retain all existing copyright holders and applicable third-party copyright, license, and embedded-content notices. Add a contributor only for substantive original contributions supported by the file's provenance.
- Formatting-only changes do not justify changing copyright ownership. Copyright years identify contribution years and must not be rewritten to erase historical authorship. Check file provenance before adding or changing a copyright claim.
...
- Use the checked-in `.clang-format` for selected C/C++ sources, headers, tests, and C/C++ feature `.inc` files. Run `./scripts/format.sh` to format through the pinned Docker image, or `./scripts/format.sh --check` for verification. Python, shell, and Markdown files use their native formatting; Python and shell files may still receive language-native copyright comments.
