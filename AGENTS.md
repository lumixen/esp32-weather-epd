...
- One Unity env: `lolin_d32_qemu` (board `lolin_d32`), run for every test config in `test/configs/`. The config is baked into the firmware at compile time, so `test/run_tests.sh` (executed in the container by the docker wrapper) reuses the env once per config, with each run's suites listed explicitly via the CLI filter `-f`: openmeteo (`test/configs/openmeteo.yml`, the env default pinned by `scripts/select_test_env_config.py`; runs the Open-Meteo provider and other config-independent suites), owm (`ESP32_EPD_CONFIG=test/configs/owm.yml`; runs the OWM provider suite with MeteoAlarm alerts), and owm_piggyback (`ESP32_EPD_CONFIG=test/configs/owm_piggyback.yml`; runs the fetch-executor suite with OWM weather and alerts sharing one provider). Every suite runs exactly once and only under a config that can link it. All runs always execute; the script exits non-zero if any failed. The run must end with all test cases succeeding.
...
- The env compiles `src/` together with the tests (`test_build_src = yes`); `main.cpp` is excluded via `#if !defined(PIO_UNIT_TESTING)`. New suites: create `test/src/<suite>/test_<suite>.cpp` and add a `-f <suite>` to the `test/run_tests.sh` runs it must execute under; `test_dir = test/src` keeps the rest of `test/` out of the build. Bare `pio test -e lolin_d32_qemu` collects every suite (the OWM suite will not link under the default openmeteo config); use `-f <suite>` for ad-hoc runs. The shared read-only stream helper over a String lives in `test/src/string_stream.h` (include it as `"../string_stream.h"`).
