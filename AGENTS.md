...
- One Unity env: `lolin_d32_qemu` (board `lolin_d32`), run for every test config in `test/configs/`. The config is baked into the firmware at compile time, so `test/run_tests.sh` (executed in the container by the docker wrapper) reuses the env once per config, with each run's suites listed explicitly via the CLI filter `-f`: openmeteo (`test/configs/openmeteo.yml`, the env default pinned by `scripts/select_test_env_config.py`; runs the config-independent suites plus the Open-Meteo provider suite) and owm (`ESP32_EPD_CONFIG=test/configs/owm.yml`; runs only the OWM provider suite, which needs the OWM provider sources). Every suite runs exactly once and only under a config that can link it. Both runs always execute; the script exits non-zero if either failed. The run must end with all test cases succeeding.
