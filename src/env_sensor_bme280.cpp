...
BME280EnvSensor::BME280EnvSensor()
    : bme(), i2cAddress(BME_ADDRESS), wire(0), initialized(false), powered(false), wireStarted(false) {}

BME280EnvSensor::~BME280EnvSensor() { shutdown(); }

void BME280EnvSensor::shutdown() {
  initialized = false;
  if (wireStarted) {
    wire.end();
    wireStarted = false;
